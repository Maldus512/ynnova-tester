...
#define DATABASE_FILE (APP_CONFIG_DATA_PATH "/preferences_db.json")
...
int preferences_load_double(double *value, char *key) {
    double number = 0;
...
void preferences_save_double(double *value, char *key) {
    save_number(*value, key);
}


int preferences_load_uint8(uint8_t *value, char *key) {
    double number = 0;
    if (load_number(&number, key)) {
        return -1;
    } else {
...
void preferences_save_uint8(uint8_t *value, char *key) {
...
int preferences_load_uint16(uint16_t *value, const char *key) {
    double number = 0;
    if (load_number(&number, (char *)key)) {
        return -1;
    } else {
...
void preferences_save_uint16(uint16_t *value, const char *key) {
    save_number((double)*value, (char *)key);
}


int preferences_load_uint32(uint32_t *value, char *key) {
    double number = 0;
    if (load_number(&number, key)) {
        return -1;
    } else {
...
void preferences_save_uint32(uint32_t *value, char *key) {
    save_number((double)*value, key);
}


int preferences_load_uint64(uint64_t *value, char *key) {
    double number = 0;
    if (load_number(&number, key)) {
        return -1;
    } else {
...
void preferences_save_uint64(uint64_t *value, char *key) {
    save_number(*value, key);
}


int preferences_load_blob(void *value, size_t len, char *key) {
...
void preferences_save_blob(void *value, size_t len, char *key) {
